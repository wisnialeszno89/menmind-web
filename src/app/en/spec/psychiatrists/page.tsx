      ...
      <div className="mx-auto max-w-3xl">
        <header className="mb-8">
          ...
          <p className="mt-4 text-zinc-300 leading-relaxed">
            This direction is for moments when symptoms are too strong and your body can’t carry it.
            Treatment is a tool — not weakness.
          </p>
        </header>

        <section className="rounded-2xl border border-zinc-800/70 bg-zinc-900/40 p-6 shadow-lg shadow-black/20 backdrop-blur">
          <p className="text-sm text-zinc-400">
            Location: <span className="text-zinc-200 font-medium">{cityLabel}</span>
          </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {CITIES.map((c) => {
              ...
              return (
                <Link
                  key={c.key}
                  href={`/en/spec/psychiatrists?city=${c.key}`}
                  className={[
          ...
          <div className="mt-8 rounded-2xl border border-zinc-800/70 bg-zinc-950/30 p-5">
            ...
            <p className="mt-2 text-sm text-zinc-400 leading-relaxed">
              Soon: online options + city filters + clear verification rules.
            </p>
...
export default function PsychiatristsPageEN(props: { searchParams?: { city?: string } }) {
  return (
    <Suspense>
      <Content {...props} />
    </Suspense>
